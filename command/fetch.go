...
func setFetch(ctx *Context, args []string) {
	...
	if len(lines) < 2 && len(ctx.Message.Attachments) == 0 {
		ctx.Reply("download this script: https://raw.githubusercontent.com/unixporn/trup/master/fetcher.sh and verify that it's genuine, if you trust it you can either do (reccomended) `chmod +x fetcher.sh && ./fetcher.sh`  or run this: `curl -s https://raw.githubusercontent.com/unixporn/trup/master/fetcher.sh | sh` and follow the instructions. https://blog.dijit.sh/don-t-pipe-curl-to-bash>\n > NOTE: use `!setfetch update` to update individual values")
		return
	}
